...
contract RevertOnRevertInvariant is StdInvariant, Test {
    ...
    function invariant_protocolMustHaveMoreValueThatTotalSupplyDollars() public view {
        ...
        uint256 wethDeposted = ERC20Mock(weth).balanceOf(address(kscEngine));
        uint256 wbtcDeposited = ERC20Mock(wbtc).balanceOf(address(kscEngine));

        uint256 wethValue = kscEngine.getUSDConversionRate(weth, wethDeposted);
    ...
    function invariant_gettersCantRevert() public view {
        kscEngine.getAdditionalFeedPrecision();
        kscEngine.getCollateralTokens();
        kscEngine.getLiquidationBonus();
        kscEngine.getLiquidationThreshold();
        kscEngine.getMinHealthFactor();
        kscEngine.getPrecisionFactor();
        kscEngine.getKSCContractAddress();
    }
}
